...
//The sort method calls the partition method to partition the array around a pivot element, and then recursively sorts the left and right sub-arrays around the pivot.
//The partition method chooses the last element in the array as the pivot, and then traverses the array from the start index to the end-1 index. The elements smaller than the pivot are swapped to the left side of the array. The pivot is then swapped with the element at the next index of the smaller element, so that the pivot is in its correct position. The method returns the pivot index.
